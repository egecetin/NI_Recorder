function [ui, handles]=maincalc(event, handles)
    ...
    if isempty(databuff) % Sadece ilk çaðýrmada boþ olacak
        ...
        for i=1:handles.nchannel
            ...
            title(local.axes{handles.nchannel-i+1,1},handles.channels{(handles.nchannel-i+1)});
            ylabel(local.axes{handles.nchannel-i+1,1},'Voltage [V]');
            ...
            ylabel(local.axes{handles.nchannel-i+1,2},'Frequency [Hz]');

            % - Init freq
            freqbuff{i}=zeros(handles.wlen/2+1,(floor((handles.fs*handles.time-handles.wlen)/(handles.wlen-handles.overlap))+1)*handles.maxstep);
            ...
            try
                ...
                cmap=cmocean(handles.map);
                colormap(local.axes{handles.nchannel-i+1,2},cmap);
            end
        ...
        if handles.filestat
            ...
            for i=1:handles.nchannel
                filelist{i}=fopen([fullpath '_' handles.channels{i} '.txt'],'W');
                dstr=datestr(datetime('now'),'yyyy.mm.dd HH:MM:SS');
                fprintf(filelist{i},'%s %s %d\n',handles.channels{i},dstr,handles.fs);
            end
        end
    ...
    if handles.filestat
        ...
        if abs(datenow)>handles.timelimit
            filetimer=datetime('now');
            dstr=datestr(datetime('now'),'yyyymmddHHMM');
            fullpath=[handles.root '\' dstr(1:4) '\' dstr(5:6) '\' dstr(7:8) '\' dstr(9:10) '\' dstr(1:8) '_' dstr(9:12)];
            mkdir(fullpath(1:end-13));
            for i=1:handles.nchannel
                fclose(filelist{i});
                pause(0.001);
                filelist{i}=fopen([fullpath handles.channels{i} '.txt'],'W');
                dstr=datestr(datetime('now'),'yyyy.mm.dd HH:MM:SS');
                fprintf(filelist{i},'%s %s %d\n',handles.channels{i},dstr,handles.fs);
            end
        end
    ...
    for i=1:handles.nchannel
        k=floor(size(freqbuff{i},2)/handles.maxstep*(handles.maxstep-1));
    ...
    % - Check time
    timer=toc();
    if timer-oldtimer>handles.time*0.95
